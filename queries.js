...
function queryServer(request,response,search) {
    ...
    else if(search == "?polls")
        pollsAPICallback(response);
}

function pollsAPICallback(response) {
    requestString = "http://elections.huffingtonpost.com/pollster/api/polls.json?page=1&state=CA&after=2016-05-20&topic=2016-president-dem-primary";
    request(requestString, function (error, resp, body) {
        if (!error && resp.statusCode == 200) {
            listResponse(body,response);
        }
        else {
            console.log("huffpo error", error);
        }
    });
} 
