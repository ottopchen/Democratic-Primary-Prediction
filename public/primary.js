...
makeRequest("polls", pollsCallback);

function pollsCallback(resp) {
    var pollsList = JSON.parse(resp);
    poll1 = pollsList[0];
    poll2 = pollsList[1];
    poll3 = pollsList[2];

    document.getElementById("poll1name").textContent = poll1.pollster + ", " + poll1.end_date;
    document.getElementById("poll2name").textContent = poll2.pollster + ", " + poll2.end_date;
    document.getElementById("poll3name").textContent = poll3.pollster + ", " + poll3.end_date;
    document.getElementById("votes1").textContent = "Hillary:" + poll1.questions[0].subpopulations[0].responses[0].value + " Bernie:" + poll1.questions[0].subpopulations[0].responses[1].value;
    document.getElementById("votes2").textContent = "Hillary:" + poll2.questions[0].subpopulations[0].responses[0].value + " Bernie:" + poll2.questions[0].subpopulations[0].responses[1].value;
    document.getElementById("votes3").textContent = "Hillary:" + poll3.questions[0].subpopulations[0].responses[0].value + " Bernie:" + poll3.questions[0].subpopulations[0].responses[1].value;
}
...
function drawModel(resp) {
    ...
    bernieDelegates = Math.round(percentBernieVotes*475);

    document.getElementById("votes4").textContent="Hillary:" + Math.round((1 - percentBernieVotes) * 100) + " Bernie:" + Math.round(percentBernieVotes * 100);
    document.getElementById("votes5").textContent="Hillary:" + (475 - bernieDelegates)  + " Bernie:" + bernieDelegates;
    ...
    updateScale(m);
    for (i=0; i < bernieRatio.length; i++)
    console.log("District " + (i + 1) + " Hillary " + (100 - (Math.round(bernieRatio[i] * 100))) + " Bernie " + (Math.round(bernieRatio[i] * 100)));
}
